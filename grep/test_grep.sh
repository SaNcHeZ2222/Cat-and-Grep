...
# TODO написать тест для -s

for file in $TEST_DATA_FOLDER/*.txt; do
  for flag in -e -i -v -c -l -n -h -o; do
    for my_pattern_search in asd '[0-9]' '[A-Z]'; do
...
for file in $TEST_DATA_FOLDER/*.txt; do
      for file2 in $TEST_DATA_FOLDER/*.txt; do
        echo "Тест с флагом -f с параметром pattern.txt в файле $file и файле2 $file2"

        # Вывод команды grep
        grep_command="grep -f pattern.txt $file $file2"
        echo " "
        echo "Команда GREP: $grep_command"
        grep_output=$(grep -f pattern.txt $file $file2)
        echo "grep Вывод:"
        echo "$grep_output"

        # Вывод команды s21_grep
        s21_grep_command="./s21_grep -f pattern.txt $file $file2"
        echo " "
        echo "Команда S21_GREP: $s21_grep_command"
        s21_grep_output=$(./s21_grep -f pattern.txt $file $file2)
        echo "s21_grep Вывод:"
        echo "$s21_grep_output"
        ...
        echo "Тест ВЫПОЛНЕН"
        passed_tests=$((passed_tests + 1))
        passed_scenarios+=("С флагом -f с параметром pattern.txt в файле $file и файле2 $file2")
        else
        echo " "
        echo "Тест ПРОВАЛЕН"
        failed_tests=$((failed_tests + 1))
        failed_scenarios+=("С флагом -f с параметром pattern.txt в файле $file и файле2 $file2")
        fi
